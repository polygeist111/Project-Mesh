...
fun UUID.matchesMask(mask: UUID): Boolean {
    if(mask.mostSignificantBits != mostSignificantBits) {
        Log.d("UUID-UTIL", "Most sig bits: $mostSignificantBits")
        Log.d("UUID-UTIL", "Most sig bit.mask: ${mask.mostSignificantBits}")
        return false
    }
    ...
    val maskLeastSigBitsWithoutPort = mask.leastSignificantBits.shr(16).shl(16)

    Log.d("UUID-UTIL", "Least sig bits comparison: UUID=${uuidLeastSigBitsWithoutPort}, Mask=${maskLeastSigBitsWithoutPort}")

    return maskLeastSigBitsWithoutPort == uuidLeastSigBitsWithoutPort
}